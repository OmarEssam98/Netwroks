...
void MyMessage_Base::setId(int id)  //Set ID for Header
{
    this->Header[0]=id;
}

void MyMessage_Base::setTime(int time)    //Set Time for Header
{
    this->Header[1]=time;
}

int MyMessage_Base::getId() const   //Get Header ID
{
    return this->Header[0];
}

int MyMessage_Base::getTime() const  //Get Header Time
{
...
std::string MyMessageDescriptor::getFieldValueAsString(void *object, int field, int i) const
{
    ...
    switch (field) {
        ...
        //case 3: {pp->getMycheckbits()->to_string();}
        default: return "";
    }
}
